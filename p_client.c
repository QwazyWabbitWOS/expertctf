...
clientreconnectinfo_t	clientreconnectinfo[6];
...
void PutClientInServer (edict_t *ent)
{
	...
	// Spider: Auto-reconnect
//	clientid = getEntOffset(ent) - 1;
//	if(clientinfo[clientid].inuse == false)
//		StuffCmd(ent, va("connect %s", SvServerAddress->string));
...
void ClientUserinfoChanged (edict_t *ent, char *userinfo)
{
	...
	if (teamplayEnabled() && expflags & EXPERT_ENFORCED_TEAMS) {
		...
		if (playerIsOnATeam(ent) &&
			Q_stricmp(ent->client->resp.oldskin, s) != 0) {
...
qboolean ClientConnect (edict_t *ent, char *userinfo)
{
	...
	if(filterban->value == 0 || filterban->value == 1)
	{
	...
	if ( strlen( value ) < 1 )
	{
	 	Info_SetValueForKey( userinfo, "rejmsg", "No Blank Names Allowed\n" );
	 	return false;
	}
	...
	if (Q_stricmp(password->string, "none") && Q_stricmp(password->string, "")) {
		if ((strcmp(password->string, value) != 0) && (strcmp(SvMasterPW->string, value) != 0))
			return false;
	}
	...
	//strncpy(clientinfo[clientid].name, value, sizeof(clientinfo[clientid].name) - 1);
	//strncpy(clientinfo[clientid].userinfo, userinfo, sizeof(clientinfo[clientid].userinfo) - 1);

	// if in list, return false; if NOT in list, return true
	//if(checkReconnectList(clientinfo[clientid].name))
	//	clientinfo[clientid].inuse = true;
	//else
	//	clientinfo[clientid].inuse = false;
...
void ClientDisconnect (edict_t *ent)
{
	...
	// Spider put player info in clientreconnectinfo_t
	clientid = getEntOffset(ent) - 1;

	for(i = 0; i < maxclients->value; i++)
	{
		if(!(clientinfo[clientid].inuse) && !(clientreconnectinfo[i].inuse))
		{
			strcpy(clientreconnectinfo[i].name, clientinfo[clientid].name);
			clientreconnectinfo[i].inuse = true;
			break;
		}
	}
...
void ClientThink (edict_t *ent, usercmd_t *ucmd)
{
	...
	float vel123;
	static int sjval;
	static float maxspeed, jumpadd, pull;

	pmove_t	pm;
	level.current_entity = ent;
	client = ent->client;

	sjval = sv_sjtoggle->value;
	pull = 	sv_pullspeed->value; 
	jumpadd = sv_maxsjspeed->value; 
	maxspeed = pull + jumpadd;
	...
	if (ucmd->impulse) {
		if (Q_stricmp(botaction->string, "kick") == 0) {
	...
	//	gi.dprintf("%f %f %f\n" , sv_pullspeed->value, sv_maxsjspeed->value, sv_pullspeed->value + sv_maxsjspeed->value);
	switch(sjval)
	{	
		
	case 0 ://sjtoggle 0 is disabled
		break;
	case 1 ://slows a player to max sjspeed +/- pullspeed of hook
		if(ucmd->upmove && ucmd->sidemove &&  ent->groundentity)
		{	
			for (i=0; i<3 ; i++)
			{				
				if (ent->velocity[i] > maxspeed )
				{
					for(vel123 = ent->velocity[i]; vel123 > maxspeed ;vel123--)
						ent->velocity[i] = vel123;
				}
				else if (ent->velocity[i] < (maxspeed * -1))
				{
					for(vel123 = ent->velocity[i]; vel123 < maxspeed * -1; vel123++)
						ent->velocity[i] = vel123;
				}
			}
			break;
		...
		break;
	case 2://this checks all player velocities and adjusts there upward 
		   //velocity if any one of them is over the maxspeed value
		if(ucmd->upmove && ucmd->sidemove &&  ent->groundentity)
		{	
			for (i=0; i<3 ; i++)
			{
					if(ent->velocity[i] > pull)
						ent->velocity[2] = ent->velocity[2] + jumpadd;
					else if (ent->velocity[i] < (pull * -1))
						ent->velocity[2] = ent->velocity[2] - (jumpadd * -1);
			}
		}
	default :
		break;
...
/***********************
	checkReconnectList();
	************************/
// false = not found; true = found, and erased
...
qboolean checkReconnectList(char *username)
{
  ...
  for(i = 0; i < maxclients->value; i++)
  {
    if(clientreconnectinfo[i].inuse && (strcmp(clientreconnectinfo[i].name, username) == 0))
    {
      clientreconnectinfo[i].inuse = false;
      return true;
    }
  }
