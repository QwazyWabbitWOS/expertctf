...
void T_Damage (edict_t *targ, edict_t *inflictor, edict_t *attacker, vec3_t dir, vec3_t point, vec3_t normal, int damage, int knockback, int dflags, int mod)
{
	...
	//  Spider: or when the server is on hold fire
	if ((int)sv_paused->value == 1 && targ->client && SvHoldFire->value == 1)
		return;
...
// do the damage
	...
	{
		//**************************test BK**************************//
		if (CTFOWNTEAMisCarrier(targ) || isCarrier(targ)){
			if(targ->max_health % targ->health < take / 3) 
			//if(take > 40) && (targ->health > 45)) 
 			{
				CTFOWNTEAMDropFlag(targ);	
			}
		//**************************** end **************************//
		}
	}
