...
#define	EXPERT_VERSION "4.0.14"	// QwazyWabbit from 4.0.4
...
// Spider: weaponflags, alternative weapons
#define EXPERT_ALT_BL							(1 << 0)		// 1
#define EXPERT_ALT_SG							(1 << 1)		// 2
#define EXPERT_ALT_SSG							(1 << 2)		// 4
#define EXPERT_ALT_MG							(1 << 3)		// 8
#define EXPERT_ALT_CG							(1 << 4)		// 16
#define EXPERT_ALT_G							(1 << 5)		// 32
#define EXPERT_ALT_GL							(1 << 6)		// 64
#define EXPERT_ALT_RL							(1 << 7)		// 128
#define EXPERT_ALT_RG							(1 << 8)		// 256
#define EXPERT_ALT_HB							(1 << 9)		// 512
#define EXPERT_ALT_BFG							(1 << 10)	// 1024
...
#define weaponflags		((int)sv_weaponflags->value)
#define utilflags		((int)sv_utilflags->value)

#define damagedrop		((int)sv_damagedropflags->value)
#define pullspeed		((float)sv_pullspeed->value)

//this fixes a speed bug with strafe,jump , and sv_airaccelerate 
#define sjtoggle		((int)sv_sjtoggle->value)
#define maxsjspeed		((float)sv_maxsjspeed->value)

#define autorestart		((int)sv_autorestart->value)
#define	mishteamaudio	((int)sv_mishteamaudio->value)
...
#define EXPERT_CHAINGUN_DAMAGE				5
...
#define EXPERT_BFG_SPEED					400
#define EXPERT_BFG_DIVIDER					10
#define	EXPERT_BFG_DAMAGE					200
// The radius for BFG lasers -- not used anywhere
//#define EXPERT_BFG_LASER_RADIUS				512

/* not used -- commented out because cvars are used
#define EXPERT_ALT_BFG_RADIUS				300
#define EXPERT_ALT_BFG_SPEED				800
#define EXPERT_ALT_BFG_DIVIDER				5
#define EXPERT_ALT_BFG_LASER_RADIUS			96
#define	EXPERT_ALT_BFG_DAMAGE				50
*/
  /*	--------------------------------
	Spider: ALTERNATIVE WEAPON STATS
	-------------------------------- */
#define EXPERT_ALT_HG_DAMAGE					70
#define EXPERT_ALT_HG_RADIUS					100
...
extern cvar_t *sv_weaponflags;	// Spider: alternative weapons
extern cvar_t *sv_utilflags;		// Obituary/GibStat/Utility flags

extern cvar_t *sv_damagedropflags; 	// Damage Drop Flag for flag mode
extern cvar_t *sv_pullspeed;		// hook speed

extern cvar_t *sv_sjtoggle;
extern cvar_t *sv_maxsjspeed;

extern cvar_t *sv_autorestart;      // var to decide whether to restart the server
extern cvar_t *restartcfg;    // var to set a default expert server cfg file
extern cvar_t *sv_mishteamaudio; //for EpertTeamAudio() on or off
...
extern cvar_t *sv_lethalmode;			//for high lethality mode gameplay--single shot deaths

/******************** bk ***************************/
//extern cvar_t *sv_fixedtime;		// server fixedtime
/******************** end bk ***********************/
...
extern cvar_t *TeamCaptureRed;
extern cvar_t *TeamCaptureBlue;
extern cvar_t *TeamListRed;
extern cvar_t *TeamListBlue;
extern cvar_t *TeamListSpec;

// Spider: Timeleft
extern cvar_t *TimeLeft;

// Spider: Admins and refs options
//extern cvar_t *SvServerAddress;
extern cvar_t *SvMasterPW;
extern cvar_t *SvMasterRCon;
extern cvar_t *SvPassworded;
extern cvar_t *SvKill;
extern cvar_t *SvHoldFire;
extern cvar_t *SvEvenTeam;

/**************************
// Spider: Auto-reconnect
**************************/
