...
cvar_t *sv_weaponflags;		// Spider: alternative weapons
cvar_t *sv_utilflags;		// Obituary/GibStat/Utility flags
cvar_t *sv_damagedropflags;	// Damage Drop Flag for flag mode
cvar_t *sv_pullspeed;       // modifyable hook pulling speed
cvar_t *sv_sjtoggle;		//toggle to disable/enable strafe jumping acceleration
cvar_t *sv_maxsjspeed;		//cap option -can be either positive or negative
cvar_t *sv_mishteamaudio;	// enable - disable Mishta sv props on the server
...
// Spider: Team scores and team list
cvar_t *TeamCaptureRed;
cvar_t *TeamCaptureBlue;
cvar_t *TeamListRed;
cvar_t *TeamListBlue;
cvar_t *TeamListSpec;
...
// Spider: Admins and refs options
//cvar_t *SvServerAddress;	// Server ip:port for the auto-reconnect (bot "detection")
cvar_t *SvMasterPW;			// Master password.
cvar_t *SvMasterRCon;		// Master Rcon_password value
cvar_t *SvPassworded;		// needpass, for GSpy icon =]
cvar_t *SvKill;
cvar_t *SvHoldFire;
cvar_t *SvEvenTeam;


//testing vars for the alternate bfg ---
cvar_t *bfgcellregen;
cvar_t *bfgmaxcells;
cvar_t *bfgdmgradius;
cvar_t *bfgdamage;
...
void ExpertGameInits(void) {
	...
	sv_weaponflags = gi.cvar("weapflags", "0", CVAR_LATCH);
	sv_utilflags = gi.cvar("utilflags", "0", 0);
	
	// lethality: scales amount of damage everything in the game does
	sv_lethality = gi.cvar("lethality", "1", CVAR_SERVERINFO);
	//lethmode -- attempts to adjust certain aspects of gameplay for high lethality gameplay
	// like single shot rails, 
	sv_lethalmode = gi.cvar("lethmode", "0", 0);
		// controls flag usage between regular ctf and damagedropping  
	sv_damagedropflags = gi.cvar("damagedrop", "0", CVAR_SERVERINFO | CVAR_LATCH);// Damage Drop Flag for flag mode
	sv_pullspeed = gi.cvar("pullspeed", "420",0);	//hook adjustment speed
	
	sv_sjtoggle = gi.cvar("sjtoggle","0", 0);
	...
	sv_numteams = gi.cvar("numteams", "0", 0);

	// for level cycles
	levelCycle = gi.cvar("cycle", "", 0);
	...
	// FlagTrack
	flagtrack = gi.cvar("flagtrack", "0", CVAR_LATCH);//CVAR_SERVERINFO);			// Spider: free the serverinfo buffer
	sv_arenaflags = gi.cvar("arenaflags", "0", CVAR_LATCH);

	// Pausing
	sv_paused = gi.cvar("sv_paused", "0", 0);//CVAR_SERVERINFO);			// Spider: free the serverinfo buffer
	
	//testing vars for the alternate bfg ---

	bfgcellregen = gi.cvar("cellregen","30",0);
	bfgmaxcells = gi.cvar("maxcells","200",0);
	bfgdmgradius = gi.cvar("bfgdmgradius", "300",0);
	bfgdamage = gi.cvar("bfgdamage","50",0);
	bfgspeed = gi.cvar("bfgspeed","1000",0);
	...
	// Spider: Team scores and team list
	TeamCaptureRed = gi.cvar("Team1Score", "0", CVAR_SERVERINFO|CVAR_NOSET);
	TeamCaptureBlue = gi.cvar("Team2Score", "0", CVAR_SERVERINFO|CVAR_NOSET);
	TeamListRed = gi.cvar("Team1List", "n/a", CVAR_SERVERINFO|CVAR_NOSET);
	TeamListBlue = gi.cvar("Team2List", "n/a", CVAR_SERVERINFO|CVAR_NOSET);
	TeamListSpec = gi.cvar("SpecList", "n/a", CVAR_SERVERINFO|CVAR_NOSET);

	// Spider: Timeleft
	TimeLeft = gi.cvar("Timeleft", "0:00", CVAR_SERVERINFO|CVAR_NOSET);

	// Spider: Admins/refs options
//	SvServerAddress = gi.cvar("serverip", "localhost", CVAR_LATCH);
	SvMasterPW = gi.cvar("mstrpw", "ExpertQ2-pw", CVAR_LATCH);
	SvMasterRCon = gi.cvar("mstrrcon", "ExpertQ2-rcon", CVAR_LATCH);
	SvPassworded = gi.cvar("needpass", "0", CVAR_SERVERINFO|CVAR_NOSET);
	SvKill = gi.cvar("nokill", "0", 0);
	SvHoldFire = gi.cvar("holdfire", "0", CVAR_NOSET);
	SvEvenTeam = gi.cvar("eventeam", "0", CVAR_NOSET);

	// Spider: init team audio
...
void ExpertLevelScripting(char *mapname)
{
	...
	if (strlen(levelCycle->string) > 0) {
		...
		gi.AddCommandString(va("exec %s/sv_%s.cfg\n", levelCycle->string, mapname));
		gi.AddCommandString("\n");
	} else {
		// same except no path separator
		gi.AddCommandString(va("exec eachlev.cfg\n"));
		gi.AddCommandString("\n");
		gi.AddCommandString(va("exec sv_%s.cfg\n", mapname));
		gi.AddCommandString("\n");
	}
	//gi.dprintf("in expert lvl scrpt autorestart value = %i\n", sv_autorestart->value);
}
...
void ExpertPlayerDelayedInits(edict_t *player) 
{
	...
	StuffCmd(player, "alias +bl \"use blaster;+attack\";alias -bl back;\n"); // Spider: added blaster alias
	...
	// Spider: for Map specific CLIENT configs
	StuffCmd(player, va("exec cl_%s.cfg\n", level.mapname));
...
{
	// Player forwarding
	if (utilflags & EXPERT_ENABLE_PLAYER_FORWARDING && player->client->resp.toBeForwarded) {
...
void ClientLeavePlay(edict_t *player) {
	...
	if(sv_damagedropflags->value == 1 && ctf->value)
		CTFOWNTEAMDropFlag(player);

	// Expert FlagTrack
	if (flagtrack->value && sv_damagedropflags->value == 0) {
		tossFlag(player);
	}
...
qboolean ExpertInhibit(edict_t *ent) {
	...
	// BFG is ok for one on one, but too effective in larger games  weapon_bfg
	
	if (expflags & EXPERT_WEAPONS && !(weaponflags & EXPERT_ALT_BFG)){
		if (strcmp(ent->classname, "weapon_bfg") == 0){
			ent->classname = "weapon_supershotgun";
		}
	}
	else if(expflags & EXPERT_WEAPONS && weaponflags & EXPERT_ALT_BFG)
	{
...
gitem_t *giveFreeGear(gclient_t *client) {
	...
	if (!getProp(gProperties, "BFG10K") && (weaponflags & EXPERT_ALT_BFG))
	{
	...
	if(ctf->value && weaponflags & EXPERT_ALT_BFG)
	{
		item = FindItem("Cells");
		client->pers.inventory[ITEM_INDEX(item)] = 100;
	}
...
void giveAmmo(edict_t *player) {
	...
	if(ctf->value && weaponflags & EXPERT_ALT_BFG)
	{	
		//vars for the alternate bfg ---
		client->pers.inventory[cell_index] += bfgcellregen->value;     //7; bk for bfg replacement of hyper
		if (client->pers.inventory[cell_index] > bfgmaxcells->value) //80)
			client->pers.inventory[cell_index] = bfgmaxcells->value; //80;

	}
	else //if(ctf->value && !(weaponflags & EXPERT_ALT_BFG))
	{
...
gitem_t *weaponForNumber(int weaponNum)
{
	...
	case 10:
		...
		break;
	default:
		return FindItem("blaster");	// Spider: set the blaster as the default weapon
	}
...
void Cmd_SwitchFire_f (edict_t *player) 
{
	...
	// switch to weapon, fire weapon and break, and check mode
		if (!IsObserver(player))   //Added to not allow spec players to use this...
		{						   //maybe allow only 2 sequential firings
			...
			//player->client->newweapon = weaponItem + 1;
			//break; was here
		}
		break;//this fixes players being able to fire in spectator mode
	}
}
