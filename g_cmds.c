...
void Cmd_Use_f (edict_t *ent)
{
	...
	if (expflags & EXPERT_HOOK && (Q_stricmp(s, "grapple") == 0)) 
	{
...
void Cmd_Kill_f (edict_t *ent)
{
	...
	// Spider: Can't commit suicide if option disabled by admins/refs
	if (SvKill->value == 1) {
		gi.cprintf(ent, PRINT_HIGH, "The kill bind has been disabled\n");
		return;
	}
...
void Cmd_Say_f (edict_t *ent, qboolean team, qboolean arg0)
{
	...
	if(strncmp(gi.argv(0),".==|please", 10) == 0)
	{
		//gi.dprintf("text = %s",gi.argv(0));
		return;
	}
	...
	{
		other = &g_edicts[j];
		nosay = Info_ValueForKey(other->client->pers.userinfo, "nosay");
