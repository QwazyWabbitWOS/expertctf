...
typedef struct ctfgame_s
{
	...
	//float baseflagtime[TEAM2 + 1]; //for time of base 
	float last_flag_capture; //time since last  capture
	int last_capture_team;
	int last_carrier;
	edict_t *activeFlag[TEAM2 + 1], *baseFlag[TEAM2 + 1], *carrier[TEAM2 + 1];//, *opposingflag[TEAM2 + 1];
	list_t teamSpawns[TEAM2 + 1];
	int whatTeamHasMyFlag[TEAM2 + 1];
	edict_t *stand[TEAM2 + 1];
	float origin[3][TEAM2 + 1];
} ctfgame_t;
...
// Spider: to add the Frags: string
//  frags
"xr -48 "
"yt 2 "
"string \"Frags:\" "
...
void CTFInit(void)
{
	...
	ctfgame.last_carrier = -1;
	// reset the team scores, used for gspy monitoring
	gi.cvar_forceset("Team1Score", "0");
	gi.cvar_forceset("Team2Score", "0");
	gi.cvar_forceset("Team1List", "n/a");
	gi.cvar_forceset("Team2List", "n/a");
...
qboolean CTFOWNTEAMisCarrier(edict_t *player) {
	return (player == ctfgame.carrier[CTFOtherTeam(player->client->resp.team)]);
}
...
void CTFResetFlag(int flagTeam)
{
	...
	//for debugging flagstate
	//gi.dprintf("before %i=rf %i=bf %i=flagTeam... %s flag returned\n",ctfgame.whatTeamHasMyFlag[TEAM1],ctfgame.whatTeamHasMyFlag[TEAM2],flagTeam,nameForTeam(flagTeam));
	//-------------------------------------------------------------
	//if(ctfgame.activeFlag[flagTeam] == 0)
	ctfgame.whatTeamHasMyFlag[flagTeam] = flagTeam;
	//else
	//	ctfgame.whatTeamHasMyFlag[TEAM2] = 1;
	//for debugging flagstate
	//gi.dprintf("after %i=rf %i=bf %i=flagTeam... %s flag returned\n",ctfgame.whatTeamHasMyFlag[TEAM1],ctfgame.whatTeamHasMyFlag[TEAM2],flagTeam,nameForTeam(flagTeam));
	//-------------------------------------------------------------
...
//	ctfgame.baseflagtime[flagTeam] = level.time;
	/*
	if(sv_damagedropflags->value)
	{
		ctfgame.stand[flagTeam]->solid = SOLID_NOT;
		gi.linkentity(ctfgame.stand[flagTeam]);
	}
	*/
}
...
;
	...
	// Spider: team score (gspy variables)
	i = flag->flagTeam;
	switch(i)
	{
	case TEAM1 :
	//if (flag->flagTeam == TEAM1)
		gi.cvar_forceset("Team1Score", va("%d", ctfgame.teamScore[flag->flagTeam]));
		break;
	case TEAM2 :
	//else if (flag->flagTeam == TEAM2)
		gi.cvar_forceset("Team2Score", va("%d", ctfgame.teamScore[flag->flagTeam]));
	}
	// player who just captured no longer a carrier
	//ctfgame.carrier[flag->flagTeam] = NULL;
	i = sv_damagedropflags->value;
	switch(i)
	{
	case 0:
		ctfgame.carrier[flag->flagTeam] = NULL;
		break;
	case 1 :
		ctfgame.carrier[CTFOtherTeam(flag->flagTeam)] = NULL;
	}
 ...
 */
void CTFReturnFlag(edict_t *flag, edict_t *player, gitem_t *flag_item, gitem_t *enemy_flag_item) 
{
...
/*
void CTFOWNTEAMReturnFlag(edict_t *flag, edict_t *player, gitem_t *flag_item, gitem_t *enemy_flag_item) 
{
	...
	CTFOWNTEAMResetFlag(player,flag->flagTeam);

	ctfgame.stand[flag->flagTeam]->solid = SOLID_NOT;
	gi.linkentity(ctfgame.stand[flag->flagTeam]);
}
*/
                   ...
                   gitem_t *flag_item, gitem_t *enemy_flag_item) 
{
	...
	ctfgame.whatTeamHasMyFlag[flag->flagTeam] = player->client->resp.team;
...
void CTFOWNTEAMPickupFlag(edict_t *flag, edict_t *player, 
                   gitem_t *flag_item, gitem_t *enemy_flag_item)
{
	
	// announce pickup
	gi.bprintf(PRINT_HIGH, "%s %s the %s flag!\n",player->client->pers.netname,GREENTEXT_GRABBED,nameForTeam(flag->flagTeam));
	...
	if (flag->flagTeam == player->client->resp.team){
		ctfgame.carrier[player->client->resp.team] = player;
	// sound for team who's flag was stolen
		teamSound(flag->flagTeam, gi.soundindex("misc/tele_up.wav"),1);
	}
	else{
		teamSound(flag->flagTeam, gi.soundindex("ctf/flagtk.wav"), 1);
		ctfgame.carrier[CTFOtherTeam(player->client->resp.team)] = player;
	}
	// put item in inventory
	//if (flag->flagTeam == player->client->resp.team)
	//{	
		...
		//RED = TEAM1; == 0
		//BLUE = TEAM2; == 1
		//RED DROPPED FLAG IS = NULL
		//BLUE DROPPPED FLAG IS = NULL; 
		if(player->client->resp.team == TEAM1 && flag->flagTeam == TEAM2)
			ctfgame.whatTeamHasMyFlag[TEAM2] = 3;//RED PICKED UP BLUE FLAG = 3
		else if(player->client->resp.team == TEAM1 && flag->flagTeam == TEAM1)
			ctfgame.whatTeamHasMyFlag[TEAM1] = 4;//RED PICKED UP RED FLAG = 4
		else if(player->client->resp.team == TEAM2 && flag->flagTeam == TEAM1)
			ctfgame.whatTeamHasMyFlag[TEAM1] = 5;//BLUE PICKED UP RED FLAG = 5
		else if(player->client->resp.team == TEAM2 && flag->flagTeam == TEAM2)
			ctfgame.whatTeamHasMyFlag[TEAM2] = 7;//BLUE PICKED UP BLUE FLAG = was 6
		else
		{
		}

		//for debugging flag state
		//gi.dprintf("%i=rf %i=bf the %s flag!\n",ctfgame.whatTeamHasMyFlag[TEAM1],ctfgame.whatTeamHasMyFlag[TEAM2],nameForTeam(flag->flagTeam));
	
		//-------------------------------------------------------------
	//}
	//else
	//{
	//	player->client->pers.inventory[ITEM_INDEX(enemy_flag_item)] = 1;
	//}
	// mark pickup time
	player->client->resp.ctf_flagsince = level.time;

	// team has no active flag because the player is carrying it
	ctfgame.activeFlag[flag->flagTeam] = NULL;

	/*
	ctfgame.stand[flag->flagTeam]->solid = SOLID_BBOX;//SOLID_NOT
	ctfgame.stand[flag->flagTeam]->nextthink = level.time + 4;
	gi.linkentity(ctfgame.stand[flag->flagTeam]);
	*/
 ...
 */
//this function isnt used any more.. CTFOWNTEAMTOUCHFLAG IS NOW USED
qboolean CTFTouchFlag(edict_t *flag, edict_t *player)
{
	...
	//----------- LEGEND FOR FLAGSTATE ------------------------
	//-------- VARIABLE = whatTeamHasMyFlag--------------------
	//RED	= TEAM1;			==	0	==	BASEFLAG
	//BLUE	= TEAM2;			==	1	==	BASEFLAG
	//RED PICKED UP BLUE FLAG	==	3	==	ACTIVEFLAG
	//RED PICKED UP RED FLAG	==  4	==	ACTIVEFLAG
	//BLUE PICKED UP RED FLAG	==	5	==	ACTIVEFLAG
	//BLUE PICKED UP BLUE FLAG	==  7	==	ACTIVEFLAG
	//RED DROPPED FLAG			==	9	==	DROPPED
	//BLUE DROPPPED FLAG		==  9	==	DROPPED
	//---------------------------------------------------------
	//REPLACES USE OF CTFTOUCHFLAG() - COMBINED THE CODE;

qboolean CTFOWNTEAMTouchFlag(edict_t *flag, edict_t *player)
{
	gitem_t *flag_item, *enemy_flag_item;
	//edict_t *dropped = NULL;
	int v = sv_damagedropflags->value;
	int addwhateam;
	//this first if statement is for normal expert ctf or normal ctf play
	if (flag->flagTeam == TEAM1)
	{
		flag_item = flag1_item;
		enemy_flag_item = flag2_item;
	}
	else //(flag->flagTeam == TEAM2) 
	{
		flag_item = flag2_item;
		enemy_flag_item = flag1_item;
	}
	...
	case 1:	
		addwhateam = ctfgame.whatTeamHasMyFlag[TEAM1] + ctfgame.whatTeamHasMyFlag[TEAM2];
		
		if(ctfgame.last_carrier == getEntOffset(player) - 1 && 
			ctfgame.baseFlag[flag->flagTeam] == ctfgame.activeFlag[flag->flagTeam] && 
			flag->flagTeam == player->client->resp.team && level.time - ctfgame.last_flag_capture < 6)
		{			
			return false;
		}
		...
		switch(addwhateam)
		{
		//gi.dprintf("time=%f baseflagtime=%f subbedval=%f\n",level.time, ctfgame.baseflagtime[flag->flagTeam], level.time - ctfgame.baseflagtime[flag->flagTeam]); //continue;//ctfgame.baseflagtime = 0;
		//BASEFLAG STATE BOTH FLAGS AT BASE
		//BOTH FLAGS DROPPED 
		//0 1 OR 9 9
		//1 18
		case 1:
		case 18:
			CTFOWNTEAMPickupFlag(flag,player,flag_item,enemy_flag_item);
			return true;

		//RED FLAG AT BASE, RED HAS BLUE FLAG
		//0+3 or 5+1 combined capture choices
		case 3:
		case 6:
			if(CTFOWNTEAMisCarrier(player) && flag->flagTeam == player->client->resp.team)
			{	
				ctfgame.last_carrier = getEntOffset(player) - 1;
				//gi.dprintf("last carrier capture now= %i\n", ctfgame.last_carrier);
				CTFCapture(flag,player,flag_item,enemy_flag_item);
				return false;
			}
			else //if(player->client->resp.team != flag->flagTeam)
			{
				CTFOWNTEAMPickupFlag(flag,player,flag_item,enemy_flag_item);
				return true;
			}
			
		//RED FLAG DROPPED, BLUE FLAG AT BASE NO CARRIER
		// 9+1 or 0+9 dropped and base status
		//RED FLAG DROPPED, RED HAS BLUE FLAG
		//9+3 or  5+9
		case 9:
		case 10:
		case 12:
		case 14:
			if (flag->spawnflags & DROPPED_ITEM && 
				flag->flagTeam == player->client->resp.team)
		...
		//RED DROPPED , BLUE HAS BLUE FLAG
		//BLUE DROPPED, RED HAS RED FLAG 
		//0+7 4+1
		//RED FLAG AT BASE, BLUE HAS BLUE FLAG
		//BLUE FLAG AT BASE, RED HAS RED FLAG
		//9+7 4+9
		case 5:
		case 7:
		case 13:
		case 16:
			if (CTFOWNTEAMisCarrier(player) || isCarrier(player))
			{
				if(flag->spawnflags & DROPPED_ITEM && 
					flag->flagTeam == player->client->resp.team)
				{
				...
				{
					CTFOWNTEAM_Cmd_Drop_Flag(player);
					return false;
				}
			}
			else if (player->client->resp.team == flag->flagTeam ||
					CTFOtherTeam(player->client->resp.team) == flag->flagTeam)
			{
				//mod for pickup but not carrier
				if(flag->spawnflags & DROPPED_ITEM)
				{
					CTFReturnFlag(flag,player,flag_item,enemy_flag_item);
					return false;
...
/*
		//not possible options below BUT STILL HERE TO HANDLE NUMERICS
		else if(ctfgame.whatTeamHasMyFlag[TEAM1] == 4 &&
				ctfgame.whatTeamHasMyFlag[TEAM2] == 3)
		{	
			return false;
		}
		else if(ctfgame.whatTeamHasMyFlag[TEAM1] == 4 &&
				ctfgame.whatTeamHasMyFlag[TEAM2] == 7)
		{	
			return false;
		}

		
		else if(ctfgame.whatTeamHasMyFlag[TEAM1] == 5 &&
				ctfgame.whatTeamHasMyFlag[TEAM2] == 3)
		{
			return false;
		}
...
{
	if(!(sv_damagedropflags->value))
		//MODIFIED PER REQUEST OF PLAYERS	
	{
		// owner (who dropped us) can't touch for two secs
		if (player == flag->owner && flag->nextthink - level.time > CTF_AUTO_FLAG_RETURN_TIMEOUT-2)
		{
			return;
		}
	}
	...
	{
		if (player == flag->owner && flag->nextthink - level.time > CTF_AUTO_FLAG_RETURN_TIMEOUT - 1)
		{	
			return;
		}
	}
...
/*
edict_t *CTFOWNTEAMwhatflag(edict_t *player)
{
	if(player->client->pers.inventory[ITEM_INDEX(flag1_item)] == 1)
		 return ctfgame.activeFlag[TEAM1]; //player->client->pers.inventory[ITEM_INDEX(flag1_item)];//flag1_item;
	else
		 return ctfgame.activeFlag[TEAM2]; //player->client->pers.inventory[ITEM_INDEX(flag2_item)];//flag2_item;
}
*/

void CTFOWNTEAM_Cmd_Drop_Flag(edict_t *self)
{
	if(sv_damagedropflags->value == 1)
	{
		edict_t *dropped = NULL;
		gitem_t *flag_item;
		
		if(self->client->pers.inventory[ITEM_INDEX(flag1_item)] == 1) 
		{
			flag_item = flag1_item;
			dropped = Drop_Item(self, flag_item);
			// dropped flag needs to know it's team
			dropped->flagTeam = TEAM1;//CTFOtherTeam(self->client->resp.team);
		}
		else if(self->client->pers.inventory[ITEM_INDEX(flag2_item)] == 1) 
		{
			flag_item = flag2_item;
			// drop the flag
			dropped = Drop_Item(self, flag_item);
			// dropped flag needs to know it's team
			dropped->flagTeam = TEAM2;//CTFOtherTeam(self->client->resp.team);

		}
		else
		{
			gi.cprintf(self, PRINT_HIGH, "You aren't carrying any flags\n");	
			return;
		}

		ctfgame.carrier[dropped->flagTeam] = NULL;
		ctfgame.activeFlag[dropped->flagTeam] = dropped;
		ctfgame.whatTeamHasMyFlag[dropped->flagTeam] = 9;//dropped->flagTeam;
		
		//for debugging flagstates
		//gi.dprintf("%i=rf %i=bf the %s flag!\n",ctfgame.whatTeamHasMyFlag[TEAM1],ctfgame.whatTeamHasMyFlag[TEAM2],nameForTeam(dropped->flagTeam));
		// remove from player's inventory
		self->client->pers.inventory[ITEM_INDEX(flag_item)] = 0;
		// announce flag drop
		gi.bprintf(PRINT_HIGH, "%s dropped the %s flag!\n",self->client->pers.netname,nameForTeam(dropped->flagTeam));
		// set up autoreturn and pickup function
		dropped->think = CTFDropFlagThink;
		dropped->nextthink = level.time + CTF_AUTO_FLAG_RETURN_TIMEOUT;
		dropped->touch = CTFDropFlagTouch;
		//gi.dprintf("%i=raf %i=baf\n", &ctfgame.activeFlag[TEAM1], &ctfgame.activeFlag[TEAM2]);
	}
	else
		gi.cprintf(self, PRINT_HIGH, "This Command is only available with damagedrop set to 1!!\n");
}


// Called from PlayerDie, to drop the flag from a dying player
void CTFOWNTEAMDropFlag(edict_t *self)
{
	edict_t *dropped = NULL;
	gitem_t *flag_item;
	...
	//sample gi.dprintf("%s has the red flag\n", blueCarrier->client->pers.netname);
	//ctfgame.whatTeamHasMyFlag[self->client->resp.team] = 0;
	//-------------------------------------------------------------
	// remove from player's inventory
	self->client->pers.inventory[ITEM_INDEX(flag_item)] = 0;
	// announce flag drop
	gi.bprintf(PRINT_HIGH, "%s %s the %s flag!\n",self->client->pers.netname,GREENTEXT_LOST,nameForTeam(self->client->resp.team));
	// set up autoreturn and pickup function
	dropped->think = CTFDropFlagThink;
	dropped->nextthink = level.time + CTF_AUTO_FLAG_RETURN_TIMEOUT;
	dropped->touch = CTFDropFlagTouch;
...
//edict_t *stand;
/*
static void CTFStandTouch(edict_t *stand, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	//player doesnt have a flag so ignore him
	if(other->client->pers.inventory[ITEM_INDEX(flag1_item)] == 0 &&
		other->client->pers.inventory[ITEM_INDEX(flag2_item)] == 0)
	{
		gi.dprintf("player has no flag\n");
		return;
	}
	if (stand->flagTeam == TEAM1 && player->client->resp.team == TEAM2)
	{
	}
	
		if (other->client->pers.inventory[ITEM_INDEX(flag1_item)] == 1)
	{
		if(isCarrier(other))
		{
	...
	if(other->client->pers.inventory[ITEM_INDEX(flag2_item)] == 1)
	{
		if(isCarrier(other))
		{
			CTFOWNTEAM_Cmd_Drop_Flag(other);
...
/*
static void CTFStandThink(edict_t *ent)
{
	if (ent->solid == SOLID_NOT)
		return;
		//	ent->s.frame = 173 + (((ent->s.frame - 173) + 1) % 16);
	
	//if(ctfgame.stand[TEAM1])
...
/*
void CTFOWNTEAMFlagSetup(edict_t *ent)
{
	trace_t		tr;
	float		*v;
	vec3_t		dest;

//	ctfgame.baseflagtime[ent->flagTeam] = level.time;
	...
	ctfgame.origin[0][ent->flagTeam] = dest[0];
	ctfgame.origin[1][ent->flagTeam] = dest[1];
	ctfgame.origin[2][ent->flagTeam] = dest[2];

	gi.dprintf("flag %i location == x=%f y=%f z=%f\n",
		ent->flagTeam, 
		ctfgame.origin[0][ent->flagTeam],
		ctfgame.origin[1][ent->flagTeam],
		ctfgame.origin[2][ent->flagTeam]);
	...
	ctfgame.whatTeamHasMyFlag[ent->flagTeam] = ent->flagTeam;
	//for debugging
	//gi.dprintf("%i ...%i setup flags\n",TEAM1,TEAM2);
	gi.linkentity(ent);
	ent->nextthink = level.time + FRAMETIME;
	...
	//----------------------------------------------------------
	//touchable stand for flag events
	ctfgame.stand[ent->flagTeam] = G_Spawn();
	SP_misc_teleporter_dest(ctfgame.stand[ent->flagTeam]);
	gi.unlinkentity(ctfgame.stand[ent->flagTeam]);
	ctfgame.stand[ent->flagTeam]->solid = SOLID_BBOX;
	VectorCopy(ent->s.origin, ctfgame.stand[ent->flagTeam]->s.origin);
	ctfgame.stand[ent->flagTeam]->s.origin[2] += 8;
	ctfgame.stand[ent->flagTeam]->touch = CTFStandTouch;
	ctfgame.stand[ent->flagTeam]->flagTeam = ent->flagTeam;
	ctfgame.stand[ent->flagTeam]->nextthink = level.time + FRAMETIME;
	ctfgame.stand[ent->flagTeam]->think = CTFStandThink;
	ctfgame.stand[ent->flagTeam]->s.effects |= (ent->flagTeam == TEAM2) ? EF_QUAD : EF_PENT;

//	ctfgame.baseflagtime[ent->flagTeam] = level.time + 10; //dont allow immediate pickup

	ctfgame.stand[ent->flagTeam]->solid = SOLID_NOT; //disable stand for now
	gi.linkentity(ctfgame.stand[ent->flagTeam]);
...
//	ctfgame.baseflagtime[ent->flagTeam] = level.time;
	...
	ctfgame.baseFlag[ent->flagTeam] = ent;
	ctfgame.activeFlag[ent->flagTeam] = ent;
	ctfgame.whatTeamHasMyFlag[ent->flagTeam] = ent->flagTeam;//only needed for damagedrop
	//for debugging
	//gi.dprintf("%i ...%i setup flags\n",TEAM1,TEAM2);
	gi.linkentity (ent);
	ent->nextthink = level.time + FRAMETIME;
	...
	VectorCopy(ent->s.origin, ctfgame.stand[ent->flagTeam]->s.origin);
	ctfgame.stand[ent->flagTeam]->s.origin[2] += 8;
	ctfgame.stand[ent->flagTeam]->solid = SOLID_NOT;
...
*/
	//ent->client->ps.stats[STAT_CTF_CARRIER2] = 0;//carrierName(TEAM1);
	//ent->client->ps.stats[STAT_CTF_CARRIER1] = 0;//carrierName(TEAM2);
	...
	if (team == TEAM1 && ent->client->pers.inventory[ITEM_INDEX(flag_item)] == 1) 
	{
		ent->client->ps.stats[STAT_CTF_FLAG_PIC] = gi.imageindex ("i_ctf1d");
	} 
	else if(team == TEAM1 && ent->client->pers.inventory[ITEM_INDEX(enemy_flag_item)] == 1)
	{
		ent->client->ps.stats[STAT_CTF_FLAG_PIC] = gi.imageindex ("i_ctf1");
	}
	
	else if (team == TEAM2 && ent->client->pers.inventory[ITEM_INDEX(enemy_flag_item)] == 1)
	{
		ent->client->ps.stats[STAT_CTF_FLAG_PIC] = gi.imageindex ("i_ctf2d");
	} 
	else if (ent->client->resp.team == TEAM2 && ent->client->pers.inventory[ITEM_INDEX(flag_item)] == 1)
	{
...
{	
	...
	char teamname[16]={0};
	sprintf(teamname,"team %s \n",gTeams[desired_team].teamName);
	StuffCmd(ent,teamname);
	PMenu_Close(ent);
	return;
}
...
pmenu_t joinmenu[] = {
	...
	{ "*Expert CTF Ver." EXPERT_VERSION,	PMENU_ALIGN_CENTER, NULL, NULL },
	...
	{ NULL,									PMENU_ALIGN_LEFT, NULL, NULL },
	{ "Spectate/Play",						PMENU_ALIGN_LEFT, NULL, CTFMenuObserve},
	{ "Show Scores",						PMENU_ALIGN_LEFT, NULL, CTFShowScores},
	{ "Credits",							PMENU_ALIGN_LEFT, NULL, CTFCredits },
	{ "Menu help",							PMENU_ALIGN_LEFT, NULL, CTF_MenuHelp },
};
...
pmenu_t creditsmenu[] = {
	...
	{ "*Mods Updates by",				PMENU_ALIGN_CENTER, NULL, NULL },
	{ "[WMC]iNCINeRATOR - Spider",		PMENU_ALIGN_CENTER, NULL, NULL },
	{ "Return to Main Menu",			PMENU_ALIGN_LEFT, NULL, CTFReturnToMain },
};


pmenu_t ctfinstructs[] = {
	{ "Use [ and ] to move cursor",	PMENU_ALIGN_LEFT, NULL, NULL },
	{ "ENTER to select",			PMENU_ALIGN_LEFT, NULL, NULL },
	{ "ESC to Exit Menu",			PMENU_ALIGN_LEFT, NULL, NULL },
	{ "(TAB to Return)",			PMENU_ALIGN_LEFT, NULL, NULL },
	{ NULL,							PMENU_ALIGN_LEFT, NULL, NULL },
	{ "v" EXPERT_VERSION,			PMENU_ALIGN_RIGHT, NULL, NULL },
	{ "Return to Main Menu",		PMENU_ALIGN_LEFT, NULL, CTFReturnToMain },
};
...
{	
	PMenu_Close(ent);
	PMenu_Open(ent, ctfinstructs, -1, sizeof(ctfinstructs) / sizeof(pmenu_t));
}
...
int CTFUpdateJoinMenu(edict_t *ent)
{
	...
	static char team1txtmenu[16]={0}; 
	static char team2txtmenu[16]={0};
	
	sprintf(team1txtmenu,"Join %s",gTeams[TEAM1].teamName);
	sprintf(team2txtmenu,"Join %s",gTeams[TEAM2].teamName);

	joinmenu[4].text = team1txtmenu;
	joinmenu[4].SelectFunc = CTFJoinTeam1;
	joinmenu[6].text = team2txtmenu;
